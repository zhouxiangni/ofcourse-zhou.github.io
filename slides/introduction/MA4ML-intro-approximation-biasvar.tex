...
\biz
\item 
We show first that   $\e[(Y-f^*(X) )  h(X) ]=0$ is true  for any function $h$.
...
minimizing in functional space
...
\begin{equation}
 \begin{split}
\e_{X,Y}(  |f(X)-Y|^2    )
 & = \e_{X,Y} (  |f^*(X)-Y|^2   ) + \e_X\left[ \abs{f(X)-f^*(X)} ^2 \right]
 \\
&=\mbox{Bayes error} + \mbox{model error}
\end{split}
\end{equation}
where $f^*(x)=\e(Y \vert X=x)$ is called {\bf Bayes rule}.
}
...
\frame{
\biz
\item Loss function $\ell(y, \hat{y}):  \Ycal  \times \Ycal \to \Real^+$
\biz
\item  $L_2$, $L_1$ norm
\item  0-1 loss:  $\ell(y,\hat{y})= I( y\neq \hat{y})$.
\item  ...
\eiz
\item $\Ecal (f) = \e \ell (Y, f(X))$
\eiz
...
\min_{f\in \Hcal} \frac{1}{N} \sum_{(x_i,y_i)\in \Dcal}\ell(y_i,f(x_i))$$
  The {\bf training error} (empirical risk) is   $\mathcal{E}(\hat{f}_\Dcal)$ .
...
points is infinity. This error may be analyzed by
...
optimization method 
 ...
 \eiz
 These three topics are not completely separate.
