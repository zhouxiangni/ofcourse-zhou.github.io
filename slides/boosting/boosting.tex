...
\item {\bf Gradient Boosting}(= Gradient Descent + Boosting) is an extension over boosting method.
...
also called base learner. $h_m$ can take real values (``confidence-rated prediction'' by Shapire and Singer) and the 
weak learner is then $\sign(h_m)$;  } $h_m(x):\mathbb{R}^p \rightarrow \{-1,1\}$ to the training data with weights $w_{m,i}$
...
$$
 ...
 \par
Consider the forward stagewise additive modeling \footnote{instead of considering all
terms $h_m$ and $\alpha_m$, $1\leq m\leq M$, simultaneously.}:
\begin{itemize}
\item Given $h_0(x),\ldots, h_{m-1}(x)$, how to find optimal $h_m(x)$ and $\alpha_m$ at stage $m$?
\end{itemize}
Then we can update 
$f_m $ as $ f_{m-1}+\alpha_m h_m(x)$
...
is Fisher consistence, so 
 for any {\color{red}{$\alpha>0$}}, the weak learner $h_m $ is consistent with  the Bayes classifier:
  $\displaystyle h_m=\argmin_{h \in {\cal H}} \sum_i w_i^{(m)} I(y_i \neq h(x_i))$.
 \footnote{ In practice, $h_m$ is not exactly this 0-1 loss minimizer
  but a minimizer of the empirical risk for 
  some Fisher consistent loss function.
  }
...
\item {\bf The weak learner assumption}: 
...
In practice, the test error does not rise much as the number of iterations $m$ increases. 
Boosting algorithms are relatively immune to overfitting but it overfit slightly  after a long time of iterations.
\vspace{-0.5cm}
\begin{figure}
\centering
...
\item By $w_{m+1,i}=w_{m,i} \exp(-y_i \alpha_m  h_m(x_i))/Z_m$ and $w_{1,i}\equiv 1/n$, 
\begin{small}
\begin{eqnarray*}
...
\leq \exp(-2\gamma^2 M)
\end{align*}
where $\gamma_m = 0.5-\epsilon_m$ and $\gamma=\min_m \gamma_m $.
\end{theorem}
...
&=& (1-\epsilon_m) e^{-\alpha_m} + \epsilon_m e^{\alpha_m} = 2 \sqrt{\epsilon_m (1-\epsilon_m)}.
\end{eqnarray*}
The desired upper bound is obtained after plugging in $\gamma_m$ and $Z_m$.
...
\frame {
The following exercise is a   generalization of the above theorem.
 \begin{ex}
...
\frame{{Generalization Error}
 {\tiny
R. E. Schapire, {et al}. Boosting the margin: A new explanation for the effectiveness of voting methods.   	Ann. Stat.,  1998.
}

The generalization error is bounded by  \[
 {\Pr}_{\cal T} \left( Yf(X) \leq \theta  \right)
+ O(\sqrt{\frac{d}{n \theta^2}})
\]
for any positive $\theta$, where $d$  is the VC dimension of the hypothesis space $\mathcal{H}$
and $n$ is the number of training samples. 
$\Pr_{\cal T}$ is the prob. w.r.t. the training data.

\begin{enumerate}
\item
The first term decays exponentially in $M$ by the above exercise.
The second term is independent of the iteration number $M$.
\item $yf(x)=y \sum_m \alpha_m h_m(x)$
\footnote{ where $\sum_m \alpha_m=1$ is normalized and $h_m$ is $+1/-1$ response} is called the {\bf margin} of  example $(x,y)$: 
the value of the margin  ($\in [-1,1]$) can be interpreted as a measure of confidence in the prediction.
...
\item Compute $
\displaystyle (\alpha_m,h_m) = \argmin_{\alpha, h \in {\cal H}} \sum_{i=1}^n (r_i- \alpha h(x_i))^2$.
\begin{itemize}
\item
One can interpret
this as  to find the closest point, $\alpha_m h_m(x)$, in 
$\mathcal{H}$ to the gradient $r_i$:
the projection of the gradient
onto the space $\mathcal{H}$.
\item
or to interpret this as
...
with binary classification where $Y=\pm 1$ (softplus function): 
$$
...
\item Initialize $f_0(x)=\frac{1}{2} \log \frac{1+\bar y}{1-\bar y}$
 \begin{ex}
Assume $f(x)\equiv f_0$ be a constant
the  solve the problem 
$\min_{f_0} L(f_0)=
\sum_{i=1}^n \log(1+\exp(-2y_i f_0)$
\end{ex}

\item Compute the gradient 
w.r.t. $f(x_i)$
\footnote{Exercise!}
at $f_{m-1}$: $g_m(x_i) = -2y_i/(1+\exp(2y_if_{m-1}(x_i))) =: -\tilde y_i$
...
=\argmax_k  f_k(x)$
\item Generalization error: $GE(\f)=Pr(Y \neq G(X))$
